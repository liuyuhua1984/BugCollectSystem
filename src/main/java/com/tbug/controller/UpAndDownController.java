...
public class UpAndDownController extends BaseController {
	...
	@ResponseBody
	public Map<String, String> uploadFile(HttpSession session, HttpServletRequest request) {
		...
		if (folderName == null || folderName.equals("")) {// 上传文件夹为空
			map.put("res", "-1");
			map.put("msg", "上传文件夹为空");
			logger.error("上传文件夹为空:" + folderName);
			return map;
		}
		...
		String savePath = session.getServletContext().getRealPath("/upload/" + folderName + File.separator + myDate);
		...
		try {
			File file = new File(savePath + File.separator + userId+SUFFIX);
			...
			map.put("res", "1");
			map.put("msg", "上传成功");
			logger.error("文件上传成功::" + folderName);
	...
	public void downLoadFile(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
		...
		String fileSaveRootPath = session.getServletContext()
				.getRealPath("/upload/" + folderName + File.separator + myDate);

		try {
			File file = new File(fileSaveRootPath + File.separator + userId+SUFFIX);

			if (file.exists()) {
				// 设置响应头，控制浏览器下载该文件
				response.setHeader("content-disposition", "attachment;filename=" + URLEncoder.encode(userId+SUFFIX, "UTF-8"));
				// 读取要下载的文件，保存到文件输入流
